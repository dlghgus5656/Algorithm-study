...
# 영화감독 숌은 세상의 종말 이라는 시리즈 영화의 감독이다. 조지 루카스는 스타워즈를 만들 때, 스
# 타워즈 1, 스타워즈 2, 스타워즈 3, 스타워즈 4, 스타워즈 5, 스타워즈 6과 같이 이름을 지었고,
...
while True:
    if '666' in str(end):  # 1 N번째 수에 '666'이 포함되어 있다면(str이 아니면 무조건 1의자리부터 시작하니까)
        count += 1  # 2 카운트를 올려라
    if count == N:  # 4 카운트랑 N번째 수가 같다면
        print(end)  # 5 end를 출력하고
        break  # 6 while문을 종료해라
    end += 1  # 3 666이 포함된 수가 나올 때 까지 end를 1씩 증가 시킨다
